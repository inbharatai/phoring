"""
OASIS Twitter simulation runner.
Reads config file parameters and runs the simulation automatically.

Features:
- Waits for commands after simulation completes, keeping the environment alive
...
class UnicodeFormatter(logging.Formatter):
    """Custom formatter that converts Unicode escape sequences to characters."""
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai max_tokens warnings (not setting max_tokens is normal)."""
...
def setup_oasis_logging(log_dir: str):
    """Configure OASIS logging and set up log files."""
...
class IPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str) -> bool:
        """
        Process a single agent interview request.
    ...
    def _get_interview_result(self, agent_id: int) -> Dict[str, Any]:
        """Get new interview results from database."""
...
class TwitterSimulationRunner:
    ...
    # Twitter (includes INTERVIEW; INTERVIEW uses ManualAction for manual control)
    AVAILABLE_ACTIONS = [
    ...
    def _get_profile_path(self) -> str:
        """Get profile file path (OASIS Twitter CSV format)."""
    ...
    def _get_active_agents_for_round(
        ...
        # Filter agents based on each agent's config
        candidates = []
        for cfg in agent_configs:
            ...
            # Check whether active at current time
            if current_hour not in active_hours:
                continue
            
            # Check activity level probability
    ...
    async def run(self, max_rounds: int = None):
        ...
        if initial_posts:
            ...
            for post in initial_posts:
                ...
                try:
                    ...
                    print(f"  Warning: Failed to create post for agent {agent_id}: {e}")
        ...
        for round_num in range(total_rounds):
            ...
            # Build LLM actions for active agents
            actions = {
                agent: LLMAction()
        ...
        # Check whether to enter command-wait mode
        if self.wait_for_commands:
            print("\n" + "=" * 60)
            print("Command-wait mode - environment is running")
            print("Supported commands: interview, batch_interview, close_env")
            print("=" * 60)
            
            self.ipc_handler.update_status("alive")
            
            # Poll for commands (using _shutdown_event for graceful shutdown)
            try:
...
async def main():
    ...
    # Initialize log config (set up log files, clean up old logs)
...
def setup_signal_handlers():
    """
    Set up signal handlers to ensure graceful cleanup on SIGTERM/SIGINT
    (close database, environment, etc.).
    """
