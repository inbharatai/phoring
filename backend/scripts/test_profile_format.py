"""
Test that generated profile formats are compatible with OASIS.
Validates:
1. Twitter profiles generate correct CSV format.
2. Reddit profiles generate correct detailed JSON format.
"""
...
def test_profile_formats():
    """Test profile format generation."""
    print("=" * 60)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"  Output file: {twitter_path}")
        print(f"  Row count: {len(rows)}")
        print(f"  Columns: {list(rows[0].keys())}")
        print(f"\n  Example data (first row):")
        ...
        print(f"  Output file: {reddit_path}")
        print(f"  Profile count: {len(reddit_data)}")
        print(f"  Fields: {list(reddit_data[0].keys())}")
        print(f"\n  Example data (first profile):")
...
def show_expected_formats():
    """Display expected OASIS profile formats."""
    print("\n" + "=" * 60)
    print("OASIS Profile Format Reference")
    print("=" * 60)
