"""
Simulation IPC (Inter-Process Communication) module.
Handles communication between the Flask server and simulation processes.

Uses file-based polling:
1. Flask writes commands to the commands/ directory.
2. Simulation process polls for commands and writes responses to responses/ directory.
3. Flask polls for responses and retrieves results.
"""
...
logger = get_logger('phoring.simulation_ipc')
...
class SimulationIPCClient:
    ...
    def send_command(
        ...
        # Write command file atomically (temp + rename) to prevent partial reads.
    ...
    def send_interview(
        ...
        """
        Send an agent interview command.

        Args:
            agent_id: Agent ID.
            prompt: Interview prompt.
            platform: Target platform (optional).
                - "twitter": Interview on Twitter platform.
                - "reddit": Interview on Reddit platform.
                - None: Interview on all running platforms.
            timeout: Timeout in seconds.
    ...
    def send_batch_interview(
        ...
        Args:
            ...
            platform: Default platform (optional, overrides per-interview platform).
                - "twitter": Default to Twitter platform.
                - "reddit": Default to Reddit platform.
                - None: Interview on all running platforms.
            timeout: Timeout in seconds.
...
class SimulationIPCServer:
    """
    Simulation IPC server (simulation process side).

    Polls the command directory for new commands and writes responses.
    """
    ...
    def poll_commands(self) -> Optional[IPCCommand]:
        ...
        Returns:
            IPCCommand or None if no commands are pending.
        """
    ...
    def send_response(self, response: IPCResponse):
        """
        Write a response file atomically (write to temp, then rename).

        Args:
            response: IPC response to send.
        """
        response_file = os.path.join(self.responses_dir, f"{response.command_id}.json")
        tmp_file = response_file + ".tmp"
        with open(tmp_file, 'w', encoding='utf-8') as f:
            json.dump(response.to_dict(), f, ensure_ascii=False, indent=2)
        # Atomic rename prevents the client from reading a partially-written file.
