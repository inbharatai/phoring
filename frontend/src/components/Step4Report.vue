    ...
    <div class="main-split-layout">
      ...
      <div class="left-panel report-style" ref="leftPanel">
        <div v-if="reportOutline" class="report-content-wrapper">
          ...
          <div v-if="executiveSummary" class="exec-summary-card">
            ...
            <div class="exec-summary-body" v-html="renderMarkdown(executiveSummary)"></div>
          </div>

          <!-- Multi-AI Consensus Validation Card -->
          <div v-if="consensusValidation" class="consensus-card">
            <div class="consensus-header">
              <div class="consensus-title-row">
                <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2">
                  <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path>
                  <circle cx="9" cy="7" r="4"></circle>
                  <path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"></path>
                </svg>
                <span class="consensus-title">Multi-AI Consensus Validation</span>
              </div>
              <div class="consensus-overall" :class="'consensus-' + consensusValidation.overall_consensus">
                <span class="consensus-badge">{{ consensusValidation.overall_consensus?.toUpperCase() }}</span>
                <span class="consensus-confidence">{{ Math.round(consensusValidation.overall_confidence * 100) }}% confidence</span>
              </div>
            </div>
            <div class="consensus-validators">
              <span class="validators-label">Validated by:</span>
              <span v-for="model in consensusValidation.validator_models" :key="model" class="validator-chip">{{ model }}</span>
            </div>
            <div class="consensus-predictions">
              <div v-for="(pred, pidx) in consensusValidation.predictions" :key="pidx" class="consensus-prediction">
                <div class="prediction-header">
                  <span class="prediction-num">#{{ pidx + 1 }}</span>
                  <span class="prediction-level" :class="'level-' + pred.consensus_level">
                    {{ pred.consensus_level === 'full_consensus' ? 'UNANIMOUS' : pred.consensus_level === 'majority' ? 'MAJORITY' : pred.consensus_level === 'split' ? 'SPLIT' : 'DISSENT' }}
                  </span>
                  <span class="prediction-confidence">{{ Math.round(pred.consensus_confidence * 100) }}%</span>
                </div>
                <p class="prediction-text">{{ pred.prediction_text }}</p>
                <div class="prediction-assessments">
                  <div v-for="a in pred.assessments" :key="a.validator_id" class="assessment-row">
                    <span class="assessment-model">{{ a.model_name }}</span>
                    <span class="assessment-agreement" :class="'agree-' + a.agreement">{{ a.agreement?.replace('_', ' ') }}</span>
                    <span class="assessment-score">{{ Math.round(a.confidence_score * 100) }}%</span>
                  </div>
                </div>
                <div v-if="pred.combined_risk_factors?.length" class="prediction-risks">
                  <span class="risk-label">Risks:</span>
                  <span v-for="(risk, ridx) in pred.combined_risk_factors.slice(0, 3)" :key="ridx" class="risk-tag">{{ risk }}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text"> generate{{ section.title }}...</span>
                </div>
              </div>
            </div>
          </div>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body":class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (contentgeneratecomplete, section complete) -->
...
const parseInsightForge = (text) => {
  ...
  try {
    // analyze 
    const queryMatch = text.match(/analyze:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // predictionscenario
    const reqMatch = text.match(/predictionscenario:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // data - match" prediction: X"format
    const factMatch = text.match(/ prediction:\s*(\d+)/)
    const entityMatch = text.match(/ entity:\s*(\d+)/)
    const relMatch = text.match(/relation:\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    // - complete, count
    const subQSection = text.match(/### analyze \n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    // - complete, count
    const factsSection = text.match(/### [][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    // coreentity - complete, includesummary 
    const entitySection = text.match(/### [coreentity]\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const relatedMatch = block.match(/:\s*(\d+)/)
        return {
    ...
    // relation - complete, count
    const relSection = text.match(/### [relation]\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // data
    const nodesMatch = text.match(/ node:\s*(\d+)/)
    const edgesMatch = text.match(/:\s*(\d+)/)
    const activeMatch = text.match(/current:\s*(\d+)/)
    const histMatch = text.match(/ \/:\s*(\d+)/)
    ...
    const activeSection = text.match(/### [current][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    // / - complete, count
    const histSection = text.match(/### [ \/][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
    ...
    // entity - complete, count
    const entitySection = text.match(/### [ entity]\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    // interview 
    const topicMatch = text.match(/\*\*interview:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // Interview (e.g. "5 / 9 simulation agents")
    const countMatch = text.match(/\*\*interview:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // interviewobject 
    const reasonMatch = text.match(/### interviewobject \n([\s\S]*?)(?=\n---\n|\n### interview)/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        } else if (currentName && line.trim() &&!line.match(/^not yet |^ |^ /)) {
          // row(exclude)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // 
      const bioMatch = block.match(/_:\s*([\s\S]*?)_\n/)
      if (bioMatch) {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If first item starts with "1.", apply special processing
      ...
      // answer - Twitter Reddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\* |$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Twitter Redditanswer
        const twitterMatch = answerText.match(/[Twitterplatformanswer]\n?([\s\S]*?)(?=[Redditplatformanswer]|$)/)
        const redditMatch = answerText.match(/[Redditplatformanswer]\n?([\s\S]*?)$/)
        ...
        // platformfallback ( oldformat: platform)
        if (!twitterMatch && redditMatch) {
          // Reddit answer, text default 
          if (interview.redditAnswer && interview.redditAnswer!== '( platformnot yet reply)') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch &&!redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer!== '( platformnot yet reply)') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch &&!redditMatch) {
          // platform ( oldformat), answer
    ...
    // interviewsummary
    const summaryMatch = text.match(/### interviewsummary coreopinion\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // nodeinfo(if)
    const nodesSection = text.match(/### node:\n([\s\S]*?)(?=\n###|$)/)
    if (nodesSection) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'predictionscenario: '),
      ...
      h('div', { class: 'insight-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `current (${props.result.facts.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `coreentity (${props.result.entities.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'relations' }],
          onClick: () => { activeTab.value = 'relations' }
        }, [
          h('span', { class: 'tab-label' }, `relation (${props.result.relations.length})`)
        ]),
        props.result.subQueries.length > 0 && h('button', {
          ...
          h('span', { class: 'tab-label' }, ` (${props.result.subQueries.length})`)
        ])
      ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          ...
          }, expandedFacts.value? ` ▲`: ` all ${props.result.facts.length} ▼`)
        ]),
        
        // Entities Tab
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'coreentity'),
          ...
          }, expandedEntities.value? ` ▲`: ` all ${props.result.entities.length} ▼`)
        ]),
        
        // Relations Tab
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'relation '),
          ...
          }, expandedRelations.value? ` ▲`: ` all ${props.result.relations.length} ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, ' querygenerateanalyze '),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, ' current '),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, ' coreentity'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, ' relation ')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `current (${props.result.activeFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'historical' }],
          onClick: () => { activeTab.value = 'historical' }
        }, [
          h('span', { class: 'tab-label' }, ` (${props.result.historicalFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, ` entity (${props.result.entities.length})`)
        ])
      ]),
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'current '),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          }, expandedHistorical.value? ` ▲`: ` all ${props.result.historicalFacts.length} ▼`)
        ]),
        
        // Entities Tab
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, ' entity'),
            h('span', { class: 'panel-count' }, ` ${props.result.entities.length} `)
          ]),
          ...
          ): h('div', { class: 'empty-state' }, ' entity'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Check whether platform text
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '( platformnot yet reply)' || t === '( platformnot yet reply)' || t === '[ reply]'
    }
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // failed: first returncompleteanswer, return 
      return qIdx === 0? answer: ''
    }
    
    // Checka certain whether platformanswer(filter text)
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      showTabs.value && h('div', { class: 'quicksearch-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, ` (${props.result.facts.length})`)
        ]),
        ...
        hasNodes.value && h('button', {
          ...
          h('span', { class: 'tab-label' }, `node (${props.result.nodes.length})`)
        ])
      ]),
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs':!showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          ...
          ): h('div', { class: 'empty-state' }, 'N/Aresult'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value =!expandedFacts.value }
          }, expandedFacts.value? ` ▲`: ` all ${props.result.facts.length} ▼`)
...
const activeStep = computed(() => {
  ...
  // if active, returnlast done step
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]
  
  // otherwisereturnfirststep
  return steps[0] || { noLabel: '--', title: ' start', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // title(## xxx), sectiontitlealready layer 
  ...
  // Processline break - row, line break <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
            ...
            ;(async () => {
              try {
                ...
                if (reportRes?.data?.executive_summary) {
                  executiveSummary.value = reportRes.data.executive_summary
                }
                if (reportRes?.data?.consensus_validation && reportRes.data.consensus_validation.total_predictions > 0) {
                  consensusValidation.value = reportRes.data.consensus_validation
                }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // if task already complete,; otherwise newlog
            if (isComplete.value) {
...
// content - LLM response sectioncontent
const extractFinalContent = (response) => {
  ...
  // ifcontent includemarkdownformat, removethink return
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // remove Thought: think 
...
.exec-summary-body ul,
.exec-summary-body p { margin: 0; padding: 3px 0; }
.exec-summary-body li {
  border-bottom: 1px solid #1F2937;
  padding: 4px 0;
}
.exec-summary-body li:last-child { border-bottom: none; }
...
.consensus-strong .consensus-badge { background: #065F46; color: #6EE7B7; }
.consensus-moderate .consensus-badge { background: #78350F; color: #FCD34D; }
.consensus-weak .consensus-badge { background: #7C2D12; color: #FDBA74; }
.consensus-mixed .consensus-badge { background: #7F1D1D; color: #FCA5A5; }
...
.consensus-prediction:last-child { border-bottom: none; }
.prediction-header {
  display: flex;
  align-items: center;
  gap: 8px;
...
.level-full_consensus { background: #065F46; color: #6EE7B7; }
.level-majority { background: #1E3A5F; color: #93C5FD; }
.level-split { background: #78350F; color: #FCD34D; }
.level-dissent { background: #7F1D1D; color: #FCA5A5; }
...
.agree-agree { background: #065F46; color: #6EE7B7; }
.agree-partially_agree { background: #1E3A5F; color: #93C5FD; }
.agree-disagree { background: #7F1D1D; color: #FCA5A5; }
.agree-error { background: #374151; color: #6B7280; }
.assessment-score {
  color: #6B7280;
  font-family: 'SF Mono', monospace;
}
...
@keyframes pulse-dot {
  ...
  50% {
    box-shadow: 0 0 0 5px rgba(99, 140, 255, 0.1);
  }
}
...
.generated-content:deep(.md-h2) { font-size: 20px; border-bottom: 1px solid rgba(255, 255, 255, 0.06); padding-bottom: 8px; }
...
.right-panel {
  ...
  /* Functional palette (dark theme) */
  --wf-border: rgba(255, 255, 255, 0.08);
  --wf-divider: rgba(255, 255, 255, 0.04);

  --wf-active-bg: rgba(99, 140, 255, 0.06);
  --wf-active-border: rgba(99, 140, 255, 0.25);
  --wf-active-dot: #638cff;
  --wf-active-text: #638cff;

  --wf-done-bg: rgba(52, 211, 153, 0.04);
  --wf-done-border: rgba(52, 211, 153, 0.15);
  --wf-done-dot: #34d399;

  --wf-muted-dot: rgba(255, 255, 255, 0.15);
  --wf-todo-text: rgba(255, 255, 255, 0.25);
}
