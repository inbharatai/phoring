<template>
  ...
  >
    ...
    <div class="section-header">
      ...
      <span class="section-title"> record</span>
      <div class="section-line"></div>
    </div>
      ...
      >
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-empty" v-else>
            ...
            <span class="empty-file-text"> file</span>
          </div>
        </div>

        <!-- title( simulation requirement 20 title) -->
        <h3 class="card-title">{{ getSimulationTitle(project.simulation_requirement) }}</h3>

        <!-- description(simulationrequirementcomplete) -->
    ...
    <div v-if="loading" class="loading-state">
      ...
      <span class="loading-text">load...</span>
    </div>
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                ...
                <div class="modal-empty" v-else> file</div>
              </div>
            </div>
            ...
            <div class="modal-actions">
              ...
              >
                ...
                <span class="btn-icon">◈</span>
                <span class="btn-text">environment </span>
              </button>
              <button 
              ...
              >
                ...
                <span class="btn-icon">◆</span>
                <span class="btn-text">analyzereport</span>
              </button>
            </div>
            <!-- hint -->
            <div class="modal-playback-hint">
              <span class="hint-text">Step3"start simulation" Step5" " row, </span>
            </div>
          </div>
        </div>
      </Transition>
...
const selectedProject = ref(null) // current project()
let observer = null
let isAnimating = false //, prevent 
let expandDebounceTimer = null // 
let pendingState = null // record run status
...
// simulationrequirementgeneratetitle( 20)
const getSimulationTitle = (requirement) => {
  if (!requirement) return 'N/Asimulation'
  const title = requirement.slice(0, 20)
  return requirement.length > 20? title + '...': title
}

// format simulation_id ( 6)
...
// format round (currentround/ round)
...
// environmentconfigpage(Simulation)
...
const loadHistory = async () => {
  try {
    ...
    console.error('load projectfailed:', error)
    projects.value = []
  } finally {
    loading.value = false
  }
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update run status( whether record new status)
        ...
        // if, recordstatus, finish Process
        if (isAnimating) return
        
        // if status currentstatus, Process
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check rowstatuswhether row( already)
          ...
          // complete, Check whether Process status 
          setTimeout(() => {
            isAnimating = false
            
            // finish, Check whether new rowstatus
...
onMounted(async () => {
  // Ensure DOM complete loaddata
  await nextTick()
  await loadHistory()
  
  // DOM Initializeobservation 
  setTimeout(() => {
    initObserver()
  }, 100)
})

// if keep-alive, component newloaddata
onActivated(() => {
  loadHistory()
})
...
.gradient-overlay {
  ...
  bottom: 0;
  background: 
    linear-gradient(to right, rgba(10, 10, 15, 0.9) 0%, transparent 15%, transparent 85%, rgba(10, 10, 15, 0.9) 100%),
    linear-gradient(to bottom, rgba(10, 10, 15, 0.8) 0%, transparent 20%, transparent 80%, rgba(10, 10, 15, 0.8) 100%);
  pointer-events: none;
}
...
/* */
.status-icon:nth-child(1).available { color: #3B82F6; } /* graph build - */
.status-icon:nth-child(2).available { color: #F59E0B; } /* environment - */
.status-icon:nth-child(3).available { color: #10B981; } /* analyzereport - */
...
/* progress status */
.card-progress.completed { color: #10B981; } /* already complete */
.card-progress.in-progress { color: #F59E0B; } /* running */
.card-progress.not-started { color: #9CA3AF; } /* N/Astarted */
...
.modal-progress.not-started { color: #9CA3AF; background: rgba(255, 255, 255, 0.04); }
