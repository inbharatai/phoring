    ...
    <div class="main-content">
      ...
      <div class="left-panel":class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            ...
            <div class="action-buttons">
                ...
                <button class="action-btn" @click="toggleFullScreen":title="isFullScreen? ' ': ' '">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">graphdataload...</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-text"> generate</p>
            <p class="waiting-hint">generatecomplete autostartBuildgraph</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 &&!graphData" class="graph-waiting">
            ...
            <p class="waiting-text">graph build </p>
            <p class="waiting-hint">data...</p>
          </div>
      ...
      <div class="right-panel":class="{ 'hidden': isFullScreen }">
        <div class="panel-header dark-header">
          ...
          <span class="header-title">Buildworkflow</span>
        </div>
          ...
          <div class="process-phase":class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title"> generate</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  upload document, LLM analyzes document content, auto-generate opinion simulation (entity type + relation type)
                </div>
              </div>
              
              <!-- generateprogress -->
              <div class="detail-section" v-if="ontologyProgress && currentPhase === 0">
                <div class="detail-label">generateprogress</div>
              ...
              <!-- already generated info -->
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">generate entity type ({{ projectData.ontology.entity_types?.length || 0 }})</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">generate relation type ({{ projectData.ontology.relation_types?.length || 0 }})</div>
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    +{{ projectData.ontology.relation_types.length - 5 }} relation...
                  </div>
                </div>
              </div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 &&!ontologyProgress">
                <div class="waiting-hint"> generate...</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase":class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">interface </div>
                <div class="detail-content">
                   generate, document call Zep API Build knowledge graph, entity relation
                </div>
              </div>
              
              <!-- complete -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint"> generatecomplete...</div>
              </div>
              
              <!-- Buildprogress -->
              <div class="detail-section" v-if="buildProgress && currentPhase >= 1">
                <div class="detail-label">Buildprogress</div>
        ...
        <div class="project-panel">
          <div class="project-header">
            ...
            <span class="project-title">projectinfo</span>
          </div>
          <div class="project-details" v-if="projectData">
            ...
            <div class="project-item">
              <span class="item-label">projectID</span>
...
const ontologyProgress = ref(null) // generateprogress
const currentPhase = ref(-1) // -1: upload, 0: generate, 1: graph build, 2: complete
const selectedItem = ref(null) // node 
...
const statusText = computed(() => {
  if (error.value) return 'Buildfailed'
  if (currentPhase.value >= 2) return 'Build Complete'
  if (currentPhase.value === 1) return 'graph build '
  if (currentPhase.value === 0) return ' generate '
  return 'Initialize '
})
...
const goToNextStep = async () => {
  try {
    // Check for an existing simulation for this project
    let simulationId = null
    try {
      const listRes = await listSimulations(currentProjectId.value)
      const sims = listRes?.data || []
      const existing = sims.find(s => s.status !== 'failed')
      if (existing) simulationId = existing.simulation_id
    } catch (_) {}
    ...
    if (!simulationId) {
      ...
      simulationId = createRes?.data?.simulation_id
    }
...
const getPhaseStatusText = (phase) => {
  if (currentPhase.value > phase) return 'already complete'
...
// Initialize - Processnew project load already project
const initProject = async () => {
  const paramProjectId = route.params.projectId
  
  if (paramProjectId === 'new') {
    // new project: store Get upload data
    await handleNewProject()
  } else {
    // load already project
    currentProjectId.value = paramProjectId
    await loadProject()
  }
}

// Processnew project - call ontology/generate API
const handleNewProject = async () => {
  const pending = getPendingUpload()
  
  if (!pending.isPending || pending.files.length === 0) {
    error.value = ' upload file, pleasereturn new '
    loading.value = false
    return
  }
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // generatestage
    ontologyProgress.value = { message: ' uploadfile analyzing document...' }
    ...
    if (response.success) {
      ...
      error.value = response.error || ' generatefailed'
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = 'projectInitializefailed: ' + (err.message || 'unknown error')
  } finally {
    loading.value = false
  }
}

// load already projectdata
const loadProject = async () => {
  try {
    ...
    error.value = 'loadprojectfailed: ' + (err.message || 'unknown error')
  } finally {
    loading.value = false
  }
}
...
const updatePhaseByStatus = (status) => {
  switch (status) {
    ...
    case 'graph_completed':
      ...
      break
    case 'failed':
      error.value = projectData.value?.error || 'Processfailed'
      break
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      // graphdataround ( taskstatusround)
      startGraphPolling()
      
      // taskstatusround 
      startPollingTask(taskId)
    } else {
      error.value = response.error || ' graph build failed'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = ' graph build failed: ' + (err.message || 'unknown error')
    buildProgress.value = null
  }
}
...
const startGraphPolling = () => {
  ...
  // Auto-get graph data every 10 seconds
...
const fetchGraphData = async () => {
  try {
    ...
    if (projectResponse.success && projectResponse.data.graph_id) {
      ...
      // Advance phase if project is already marked complete (e.g. task lost from memory)
...
const pollTaskStatus = async (taskId) => {
  try {
    ...
    if (!response.success) {
      // Task not found (e.g. server restart wiped in-memory tasks) — fall back to project status
      try {
        const projectResponse = await getProject(currentProjectId.value)
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ graph build complete, load complete data...')
        
        stopPolling()
        stopGraphPolling()
        currentPhase.value = 2
        
        // Update progress completestatus
        buildProgress.value = {
          progress: 100,
          message: 'Build Complete, loadgraph...'
        }
        ...
        if (projectResponse.success) {
          ...
          if (projectResponse.data.graph_id) {
            console.log('📊 loadcompletegraph:', projectResponse.data.graph_id)
            await loadGraph(projectResponse.data.graph_id)
...
const renderGraph = () => {
  ...
  const edges = edgesData
    ...
    .map(e => ({
      ...
      rawData: {
        ...e,
        source_name: nodeMap[e.source_node_uuid]?.name || 'not yet ',
        target_name: nodeMap[e.target_node_uuid]?.name || 'not yet '
      }
    }))
