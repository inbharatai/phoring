...
| **Gemini API** | Primary reasoning + report generation (Gemini 2.5 Pro) and Validator-3 consensus (Gemini 2.5 Flash) via `generativelanguage.googleapis.com` | `backend/app/config.py` (`LLM_BASE_URL`, `LLM_VALIDATOR_3_*`), `backend/app/utils/llm_client.py`. See README "Multi-AI Consensus Validation" |
...
```
              ...
              │   artifacts-    │    │  simulation_runs │  │ 2.5 Flash    │
